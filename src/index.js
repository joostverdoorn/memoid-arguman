...
function transformArgument(argument) {
  ...
  function transformUser(user) {
    ...
    return [
      [uid, context['name'], user.username],
      [uid, context['type'], context['Person']]
    ];
  }
  ...
  function transformPremise(premise) {
    ...
    return [
      [pid, context['citation'], cid],
      [pid, context['author'], uid],
      [pid, context['text'], premise.text],
      ...transformUser(premise.user)
    ];
  }

  return [
    [aid, context['type'], context['CreativeWork']],
    [aid, context['author'], uid],
    [aid, context['text'], argument.title],
    ...transformUser(argument.user),
    ...flatten(argument.premises.map(transformPremise))
  ];
}
